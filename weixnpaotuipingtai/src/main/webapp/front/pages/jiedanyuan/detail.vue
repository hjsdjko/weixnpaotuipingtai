...
<script>
	export default {
		...
		methods: {
			...
			// 获取详情
			async init() {
				let res = await this.$api.info('jiedanyuan', this.id);
				this.detail = res.data;
				// 主图
				this.swiperList = this.detail.jiedanyuanPhoto ? this.detail.jiedanyuanPhoto.split(",") : [];
			},
			...
			async upCallback(mescroll) {
				let res = await this.$api.list('jiedanyuanLiuyan', {
					page: mescroll.num,
					limit: mescroll.size,
					jiedanyuanId: this.id
				});
				// 如果是第一页数据置空
