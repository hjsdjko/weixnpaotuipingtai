<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input v-model="searchForm.jiedanyuanName" type="text" placeholder="跑腿员姓名"></input>
		</view>
...
<view class="uni-product-list" :style='{"borderRadius":0,"backgroundColor":"rgba(224, 224, 224, 1)"}'>
    <view
        ...
        <view class="uni-product-title"
            ...
            {{product.jiedanyuanName}}</view>
        <view class="image-view">
            <image :style='{"borderRadius":"0px","width":"100%","height":"100%"}' mode="aspectFill"
                   class="uni-product-image" :src="product.jiedanyuanPhoto?product.jiedanyuanPhoto.split(',')[0]:''"></image>
        </view>
        <view class="uni-product-price" v-if="product.jiedanyuanNewMoney != null"
              :style='{"fontSize":"24rpx","lineHeight":"60rpx","color":"#999","textAlign":"left"}'>
            <text class="">￥{{product.jiedanyuanNewMoney}}</text>
        </view>
        <view style="display: flex;justify-content: space-between;">
            <text v-if="isAuth('jiedanyuan','修改')" class="cuIcon-edit"
                  @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="isAuth('jiedanyuan','删除')" class="cuIcon-delete"
            ...
            v-if="isAuth('jiedanyuan','新增')" class="add-btn" @click="onAddTap()">新增</button>
...
<script>
	export default {
		...
		methods: {
			//查询条件切换
			queryChange(e) {
				this.searchForm.jiedanyuanName = "";
			},
			//类别搜索
			...
			async upCallback(mescroll) {
                ...
                let res = await this.$api.list(`jiedanyuan`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('jiedanyuan', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search() {
				...
				if (this.searchForm.jiedanyuanName) {
                    searchForm['jiedanyuanName'] = this.searchForm.jiedanyuanName
                }
				if (this.searchForm.jiedanyuanName) {
					searchForm['jiedanyuanName'] = this.searchForm.jiedanyuanName
				}

                let res = await this.$api.list(`jiedanyuan`, searchForm);
				// 如果是第一页数据置空
